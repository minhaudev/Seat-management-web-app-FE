...
import {getRoomChange} from "@/services/manager/room";
...
export default function RoomDetails() {
    ...
    const getChangeData = async () => {
        ...
        if (res && res.length > 0) {
            const currentRoom = res.find((room: any) => room.roomId === roomid);
            if (currentRoom) {
                setObjectData(currentRoom.changedData || []);
                setLocalSeats(currentRoom.seatData || []);
            }
        }
    };
    ...
    return (
        <LayoutContainer
            ...
            <div
                ...
                {localSeats && Array.isArray(localSeats) ?
                    localSeats
                    ...
                    <Tooltip
                        ...
                        content={
                            <div className="text-[14px] bg-white text-gray-7 rounded-md">
                                {!localSeats.find((s) => s.id === menu.seatId)
                                    ?.user && (
                                    <div>
                                        <p
